...
public class ResourceIndexCreator {
  ...
  public void createOntologieIndexFromFile(
      List<File> files, String predicateIndex, String classIndex) {
    try {
      writeIndex = new WriteElasticSearchIndex();
      ...
      for (String cl : foundClasses) {
        if (ontToLable.containsKey(cl)) writeIndex.indexClass(cl, ontToLable.get(cl));
      }
      for (String cl : foundPredicates) {
        if (ontToLable.containsKey(cl)) writeIndex.indexProperty(cl, ontToLable.get(cl));
      }
      writeIndex.commit();
      writeIndex.close();
    } catch (Exception e) {
  ...
  public void createOntologieIndexFromUrls(
      List<String> resources, String predicateIndex, String classIndex) {
    try {
      writeIndex = new WriteElasticSearchIndex();
      writeIndex.setOntologieIndexes(predicateIndex, classIndex);
      writeIndex.createBulkProcessor();
      for (String urlstring : resources) {
        ...
        if (urlstring.endsWith(OWL)) indexTTLFileFromWeb(inputStream, OWL);
        writeIndex.commit();
      }
      writeIndex.close();
    } catch (Exception e) {
  ...
  public void createIndexFromUrls(List<String> resources, String index) {
    ...
    try {
      ...
      for (String urlstring : resources) {
        ...
        if (urlstring.endsWith(TTL + ".bz2")) indexTTLFileFromWeb(inputStream, TTL);
        if (urlstring.endsWith(NT + ".bz2")) indexTTLFileFromWeb(inputStream, NT);
        inputStream.close();
      }
      writeIndex = new WriteElasticSearchIndex();
      writeIndex.setResourceIndexes(index);
  ...
  private class OntologieStatementHandler extends RDFHandlerBase {
    ...
    public void handleStatement(Statement st) {
      ...
      if (object.equals("http://www.w3.org/2002/07/owl#Class")
          || object.equals("http://www.w3.org/2000/01/rdf-schema#Class")) foundClasses.add(subject);
      ...
      if (predicate.equals("http://www.w3.org/2000/01/rdf-schema#label")) {
        ...
        if (lang != null && lang.equals("de")) {
          if (!ontToLable.containsKey(subject)) ontToLable.put(subject, new ArrayList<>());
  ...
  private class OnlineStatementHandler extends RDFHandlerBase {
    ...
    public void handleStatement(Statement st) {
      ...
      if (predicate.equals("http://www.w3.org/1999/02/22-rdf-syntax-ns#type")) {
        ...
        // if(nameToResourceSubject.size()%1000==0)System.out.println("Mapsize
        // Resource"+nameToResourceSubject.size());
        ...
        if (!nameToProperties_subject.containsKey(subject))
          nameToProperties_subject.put(subject, predicate);
        else
          nameToProperties_subject.put(
              subject, nameToProperties_subject.get(subject) + ",," + predicate);
        if (!nameToProperties_object.containsKey(subject))
          nameToProperties_object.put(object, predicate);
        else
          nameToProperties_object.put(
              subject, nameToProperties_object.get(object) + ",," + predicate);
      } else if (!predicate.equals("http://www.w3.org/2000/01/rdf-schema#label")) {
        // if(nameToProperties_subject.size()%1000==0)System.out.println("Mapsize
        // Properties"+nameToProperties_subject.size());
        if (!nameToProperties_subject.containsKey(subject))
          nameToProperties_subject.put(subject, predicate);
        else
          nameToProperties_subject.put(
      ...
      if (predicate.equals("http://www.w3.org/2000/01/rdf-schema#label")
          ...
          || predicate.equals(
              "https://portal.limbo-project.org/vocab/IndoorNavigationVocab/hasName")) {
      ...
      if (nameToProperties_subject.size()
              + nameToProperties_object.size()
              + nameToLabels.size()
              + nameToResourceSubject.size()
              + nameToTypes.size()
          > 1000000) index();
      /*if(predicate.equals("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"))
